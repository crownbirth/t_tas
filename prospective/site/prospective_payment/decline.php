...
//		$result = mysql_query($sqlget);
                ...
                mysql_query('BEGIN', $tams);
                 ...
                 $update= sprintf("UPDATE  schfee_transactions SET amt='0',resp_code='".$rc."',resp_desc = '".$res."', auth_code='".$ac."', pan='".$pan."', status = 'DECLINED', name='".$name."', date_time='".$dt."', xml = '".$xmlmsg."' WHERE  ordid='".$ordid."'");
                 $rs_update = mysql_query($update, $tams) or die(mysql_error());
                 
                $query_paid= sprintf("UPDATE prospective SET payform=%s WHERE jambregid=%s", GetSQLValueString("Yes", "text"), GetSQLValueString(getSessionValue('MM_Username'), "text"));
