...
//            . "AND dc.csid = r.csid "
            ...
            . "AND s.sesid = r.sesid "
//            . "AND dc.progid=%s "
            . "AND (%s) ORDER BY r.sesid ASC, semester ASC", 
...
//            GetSQLValueString($progid, "int"), 
    ...
    for($i = 0; $i < $totalRows_cgpa; $i++, $row_cgpa = mysql_fetch_assoc($cgpa)) {
        ...
        if(getPassmark($sesn, $row_cgpa['csid'], $first) < $row_cgpa['score']) {
...
<div style="text-align:center; width:100%; font-size: 16pt">
    <div style="float:left;text-align:left; width:20%; font-size: 16pt">'.$row_rssess['sesname'].' ('.strtoupper($semester).')'.'</div>
    <div style="float:left;text-align:center; width:60%; font-size: 16pt">Summary Sheet</div>
    ...
    if(!empty($passlist)) {
        $html .= "<table width='850' class='table table-striped table-bordered'>
                ...
                <tr>
                  ...
                  <th>{$gpaName}</th>
                  <th>{$tupName}</th>
                  <th width='40%'>Remark: Courses to take or repeat</th>
                </tr>
